Shipping Security Telemetry into Splunk on Kubernetes

I wanted to write up how to deploy Splunk via Helm and GitOps through ArgoCD. This is part of a larger homelab security stack I have been building out and it has been a learning process. There was not a lot of info online about how to actually do this end to end. Sure there are the GitHub pages for the Splunk operator and the OTel collector chart, but as a beginner jumping into those docs it is pretty overwhelming. So I wanted to share what I ran into and what actually worked.

The issues I hit along the way:

    The splunkGeneralTerms flag is required as of operator 3.0.0 and the docs kind of bury it. Without it the container just refuses to start and the error message does not make it obvious why.
    The enterprise chart ships with the operator as a sub-chart dependency. If you are running the operator as its own ArgoCD app (which you should be) and you forget to disable it in the enterprise chart, you end up with two operators fighting each other.
    OOMKills. I set memory too low on the first attempt and Splunk got killed under any real indexing load. 2Gi is the floor.
    The self-signed cert situation with HEC inside the cluster took a while to figure out.
    I also ended up with four HEC tokens from trial and error runs and had to work out which one was actually being used. More on that below.

The Stack

This post is one piece of a larger homelab project. The full thing is a 19-phase build documented at HomeLab-Project. By the time Splunk goes in at Phase 9 the cluster already has a fair bit running.
...
The whole deployment is three files committed to Git and synced by ArgoCD:
    ...
    splunk-operator — the operator itself
    splunk-enterprise — the actual Splunk instance
    splunk-otel-collector — the OTel agent that ships logs to Splunk via HEC
